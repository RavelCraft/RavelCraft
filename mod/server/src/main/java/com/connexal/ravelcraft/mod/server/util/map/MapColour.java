...
public enum MapColour {
    ...
    public static int[][] generateColourPalette() {
        int[][] palette = new int[values().length * 4][];
        int index = 0;
        for (MapColour colour : values()) {
            palette[index++] = colour.getMultiplied(0.71f);
            palette[index++] = colour.getMultiplied(0.86f);
            palette[index++] = colour.getMultiplied(1f);
            palette[index++] = colour.getMultiplied(0.53f);
        }
        return palette;
    }
    ...
    MapColour(int red, int green, int blue, int alpha) {
        this.red = red;
        this.green = green;
        this.blue = blue;
        this.alpha = alpha;
    ...
    MapColour(int red, int green, int blue) {
        this(red, green, blue, 255);
    }

    public int[] getMultiplied(float value) {
        return new int[]{ (int) (value * this.red), (int) (value * this.green), (int) (value * this.blue), this.alpha };
    }
}
